...
go
create procedure delete_quocgia (
@maqg varchar(5)
) as begin
...
@mact numeric(18,0), @hoten nvarchar(100), @vitri nvarchar(20), @ngaysinh datetime, @diachi nvarchar(200), @maclb varchar(5), @maqg varchar(5), @so int
) as begin
...
go
create procedure select_cauthu (
@mact numeric(18,0)
) as begin
	select * from CAUTHU_DatNT;
end;
go exec select_cauthu;


-- Bảng HLV
drop proc select_hlv;
go create proc select_hlv 
as begin
...
go exec insert_hlv 'HLV50', N'Nguyễn Tiến Đạt - 4242', null, null, null, 'VN';
-- 
drop proc update_hlv;
go create proc update_hlv (
@mahlv varchar(5), @tenhlv nvarchar(100), @ngaysinh datetime, @diachi nvarchar(200), @dienthoai nvarchar(20), @maqg varchar(5)
) as begin
...
go exec update_hlv 'HLV50', N'Nguyễn Tiến Đạt - 4242', null, null, null, 'ANH';
--
drop proc delete_hlv;
go create proc delete_hlv (@mahlv varchar(5)) 
as begin
	delete HUANLUYENVIEN_DatNT where MAHLV_4242 = @mahlv;
end;
go exec delete_hlv 'HLV50';
...
go create proc select_hlvclb 
as begin
	select * from HLV_CLB_DatNT;
end;
go exec select_hlvclb;
--
drop proc insert_hlvclb;
go create proc insert_hlvclb(
...
go exec insert_hlvclb 'HLV50', 'HAGL', 'HLV thể lực';
-- 
drop proc update_hlvclb;
go create proc update_hlvclb(
	@mahlv varchar(5), @maclb varchar(5), @vaitro nvarchar(100)
) as begin
	update HLV_CLB_DatNT set MACLB_4242=@maclb, VAITRO_4242=@vaitro 
	where MAHLV_4242 = @mahlv;
end;
go exec update_hlvclb 'HLV50', 'BBD', 'HLV thủ môn';
--
drop proc delete_hlvclb;
go create proc delete_hlvclb(
	@mahlv varchar(5)
) as begin
	delete from HLV_CLB_DatNT where MAHLV_4242 = @mahlv;
end;
go exec delete_hlvclb 'HLV50';


-- Bảng CauLacBo
drop proc select_clb;
go create proc select_clb
as begin
...
go exec insert_clb 'DTQG', N'Đội tuyển quốc gia', 'CL', 'DN';
-- 
drop proc update_clb;
go create proc update_clb (
	@maclb varchar(5), @tenclb nvarchar(100), @masan varchar(5), @matinh varchar(5)
) as begin
	update CAULACBO_DatNT set TENCLB_4242 = @tenclb, MASAN_4242 = @masan, MATINH_4242 = @matinh
	where MACLB_4242 = @maclb;
end;
go exec update_clb 'DTQG', N'Đội tuyển Việt Nam', 'CL', 'DN';
-- 
drop proc delete_clb;
go create proc delete_clb (@maclb varchar(5))
as begin
	delete CAULACBO_DatNT where MACLB_4242 = @maclb;
end;
go exec delete_clb 'DTQG';




----b. Bài tập về Trigger
----Viết các trigger có nội dung sau:
...
--	print @SO_4242;
--	print @MACLB_4242;
--	select ct.SO_4242 from CAUTHU_DatNT ct where ct.MACLB_4242 = @MACLB_4242;
...
as begin
	if exists (select * from inserted i where
		i.VITRI_4242 not in (N'Thủ môn', N'Tiền đạo', N'Tiền vệ', N'Trung vệ', N'Hậu vệ')) 
	begin
		rollback transaction;
	end;
end;
...
for insert
as begin
select * from CAUTHU_DatNT ct , inserted i where ct.MACLB_4242 = i.MACLB_4242
		and ct.SO_4242 = i.SO_4242
	--if exists (select * from CAUTHU_DatNT ct , inserted i where ct.MACLB_4242 = i.MACLB_4242
	--	and ct.SO_4242 = i.SO_4242 )
	--begin
	--	rollback transaction;
	--end;
